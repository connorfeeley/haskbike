...
dayTimes = [(addDays n refDay, TimeOfDay h 0 0) | n <- [0..2], h <- [0,2..22]]
  where refDay = fromGregorian 2023 10 01  -- Replace with reference day.
...
formatBikeCounts allCounts = Box.printBox table
  where
    ...
    col4 = Box.vcat Box.left (showFn Dull Yellow "EFit"       : [(showFn Vivid Yellow . show) e              | (a, b, c, d, e, f) <- allCounts, let s = e])
    col5 = Box.vcat Box.left (showFn Dull Yellow "EFit G5"    : [(showFn Vivid Yellow . show) f              | (a, b, c, d, e, f) <- allCounts, let s = e])
